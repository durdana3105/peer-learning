...
// Issue 1: Type Safety
createFix(
  "fix/type-safety-bypasses-1135",
  {
    "src/lib/deleteResource.ts": (c) =>
      c.replace(/\/\/ @ts-expect-error TODO: refine typing\n/g, ""),
  },
...
createFix(
  "fix/supabase-client-casting-1136",
  {
    "src/pages/Notifications.tsx": (c) =>
      c.replace(/\(supabase as any\)/g, "supabase"),
  },
...
createFix(
  "fix/gamification-rpc-forgery-1138",
  {
...
createFix(
  "fix/streak-restoration-race-condition-1140",
  {
...
createFix(
  "fix/session-creation-role-bypass-1142",
  {
...
createFix(
  "fix/chat-spoofing-timestamp-forgery-1143",
  {
...
createFix(
  "fix/unstructured-state-management-1144",
  {
    "src/pages/MentorDashboard.tsx": (c) =>
      c
        .replace(
          /useState<any>\(null\)/g,
