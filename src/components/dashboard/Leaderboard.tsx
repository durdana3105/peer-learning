...
function Leaderboard() {
  const { data: contributors = [], isLoading } = useQuery<Contributor[]>({
    ...
    queryFn: () =>
      fetch(
        "https://api.github.com/repos/durdana3105/peer-learning/contributors",
      ).then((res) => {
  ...
  return (
    <div className="mt-8 bg-zinc-900/70 border border-cyan-500/10 rounded-3xl p-8">
      <h2 className="text-3xl font-semibold mb-6">Top Contributors</h2>

      <div className="space-y-5">
        {(Array.isArray(contributors) ? contributors.slice(0, 5) : []).map(
          (user, index) => (
            <div
              key={user.login}
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-10 text-cyan-400 font-bold">#{index + 1}</div>
