...
const LeaderboardRow = memo(
  ({ entry, index, isCurrentUser }: LeaderboardRowProps) => {
    return (
      <motion.div
          ...
          <div className="flex items-center gap-2 rounded-2xl bg-cyan-500/10 px-4 py-3">
            ...
            <div>
              ...
              <h2 className="text-xl font-black text-cyan-400">
                {entry.decayed_score != null
                  ? Math.round(entry.decayed_score)
                  : entry.xp}
              </h2>
            </div>
          </div>
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.rpc(
        "get_decayed_leaderboard" as any,
        {
          decay_days: 30,
          limit_count: 50,
          p_filter: filter,
      ...
      if (user) {
        ...
        if (myData) {
          ...
          const { data: rpcRank, error: rankError } = await supabase.rpc(
            "get_user_rank",
            {
              p_user_id: user.id,
              p_filter: filter,
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050816] px-4 py-10 text-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Earn XP every time you join a live study session with peers.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-sm font-semibold text-white">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Help others by answering questions in Anonymous Doubts to gain
                XP.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Log in and stay active daily to build your streak and boost your
                rank.
              </p>
            </div>
          </div>
            ...
            {[entries[1], entries[0], entries[2]].map((entry, i) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="mt-4 text-lg font-bold">{entry.username}</h3>
        ...
        <div
          ref={listParentRef}
          className="mt-16 h-[1200px] overflow-y-auto pr-2"
        >
          <div
            style={{
        ...
        {entries.length === 0 && (
          <div className="mt-20 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="mt-6 text-3xl font-bold">No rankings yet</h2>
