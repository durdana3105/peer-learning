...
export default function PeerReviewDashboard() {
  ...
  const renderSubmissionCard = (sub: any, isMine: boolean = false) => {
    ...
    const avatar = isAnon
      ? `https://api.dicebear.com/9.x/bottts/svg?seed=${sub.id}`
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-medium ${sub.status === "pending" ? "bg-yellow-500/20 text-yellow-300" : "bg-green-500/20 text-green-300"}`}
          >
            {sub.status === "pending" ? (
              <>
              ...
              <>
                <CheckCircle className="mr-1 inline h-3 w-3" /> Reviewed
              </>
            )}
          </span>
        </div>
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            Request feedback on your projects, essays, and code snippets, or
            review others' work to help them learn.
          </p>
        </div>
          ...
          {pendingSubmissions.length === 0 ? (
            <div className="rounded-xl border border-white/5 bg-white/5 p-8 text-center backdrop-blur-md">
              ...
              <p className="text-sm text-slate-500">
                You're all caught up! Check back later for more peer review
                requests.
              </p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {pendingSubmissions.map((sub) =>
                renderSubmissionCard(sub, false),
              )}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="reviews" className="space-y-4">
          {myReviews.length === 0 ? (
            <div className="rounded-xl border border-white/5 bg-white/5 p-8 text-center backdrop-blur-md">
              ...
              <p className="text-sm text-slate-500">
                Help a peer out by reviewing a pending request.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-white">
                      Re: {review.peer_submissions?.title}
                    </h3>
                    {review.rating ? (
                  ...
                  <p className="mt-3 text-sm text-slate-300 bg-black/20 p-3 rounded-lg border border-white/5">
                    {review.feedback}
                  </p>
