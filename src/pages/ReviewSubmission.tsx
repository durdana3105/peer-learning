...
export default function ReviewSubmission() {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    const [
      { data: reviewWithProfile, error: reviewFetchError },
      { data: refreshedSubmission, error: submissionFetchError },
    ] = await Promise.all([
      supabase
        .from("peer_reviews")
        .select("*, profiles(name, avatar_url)")
        .eq("id", reviewRow.id)
        .single(),
      supabase.from("peer_submissions").select("status").eq("id", id).single(),
    ]);
    ...
    if (submissionFetchError) {
      console.error(
        "Failed to refresh submission status:",
  ...
  const handleRateReview = async (reviewId: string, ratingValue: number) => {
    ...
    if (error) {
      ...
      setReviews(
        reviews.map((r) =>
          r.id === reviewId ? { ...r, rating: ratingValue } : r,
        ),
      );
    }
  ...
  const avatar = isAnon
    ? `https://api.dicebear.com/9.x/bottts/svg?seed=${submission.id}`
    : submission.profiles?.avatar_url ||
      `https://api.dicebear.com/9.x/avataaars/svg?seed=${authorName}`;
  const contentUrlText =
    typeof submission.content_url === "string"
      ? submission.content_url.trim()
      : "";
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="rounded-2xl border border-white/10 bg-[#050816]/50 p-6 backdrop-blur-xl">
            ...
            {submission.content && (
              <div>
                <h3 className="text-sm font-medium text-slate-500 uppercase tracking-wider mb-2 flex items-center">
                  <Code className="mr-2 h-4 w-4" /> Content
                </h3>
            ...
            <div className="space-y-4">
              {reviews.map((review) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <img
                      src={
                        ...
                        `https://api.dicebear.com/9.x/avataaars/svg?seed=${review.profiles?.name}`
                      }
                      alt="Reviewer"
                  ...
                  <p className="text-sm text-slate-300 bg-black/20 p-3 rounded-lg border border-white/5 mb-3">
                    {review.feedback}
                  </p>
